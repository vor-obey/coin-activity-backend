...
// }
//
// async function getUsdtFuturesSymbols(): Promise<string[]> {
...
// }
//
// function createWsUrls(symbols: string[]): string[] {
//     const maxStreamsPerWs = 30;
//     const chunks = [];
//     for (let i = 0; i < symbols.length; i += maxStreamsPerWs) {
//         const chunk = symbols.slice(i, i + maxStreamsPerWs);
//         chunks.push(chunk.map(s => `${s}@depth20@100ms`).join('/'));
//     }
//     return chunks.map(chunk => `wss://fstream.binance.com/stream?streams=${chunk}`);
// }
//
// function subscribeToBooks(
//     symbols: string[],
//     onDensityUpdate: (data: DensityData) => void
// ) {
//     const urls = createWsUrls(symbols);
...
//
//         ws.on('open', () => {
//             console.log(`Book WS connected: ${url}`);
//         });
//
//         ws.on('message', (msg) => {
//             try {
...
//
//                 const densities = bids.map(([price, qty]) => {
//                     const p = parseFloat(price);
//                     const q = parseFloat(qty);
//                     return { density: p * q, price: p };
//                 });
//
//                 const maxBid = densities.reduce(
//                     (max, cur) => (cur.density > max.density ? cur : max),
//                     { density: 0, price: 0 }
//                 );
//
//                 maxDensityMap[symbol] = Math.max(maxDensityMap[symbol] || 0, maxBid.density);
//                 const percent = maxDensityMap[symbol] ? maxBid.density / maxDensityMap[symbol] : 0;
//
//                 onDensityUpdate({
//                     symbol,
//                     percent,
//                     maxDensity: maxBid.density,
//                     price: maxBid.price,
//                 });
//             } catch (e) {
...
//
//         ws.on('close', () => {
//             console.warn('Book WS closed, reconnecting in 5s...');
//             setTimeout(() => {
//                 subscribeToBooks(symbols, onDensityUpdate);
//             }, 5000);
//         });
//
//         sockets.push(ws);
...
//     console.log('Front client connected');
//     clients.add(ws);
//
//     ws.on('close', () => {
...
// Получить только растущие USDT-пары
